   ...
   public static void main(String[] args)
   {
      ...
      if (parser.getNumberOfSyntaxErrors() == 0)
      {
         ...
         */
         ...
         program.typeCheck(); // typecheck
         program.toCFG(); // generate CFG
         String fname = args[0];
         fname = fname.replace("../../", "");
         fname = fname.replace("input/", "");
         fname = fname.replace(".mini", "");
         program.toLLVM(fname); // generate LLVM code
         program.toARM(fname, false); // generate ARM code
         program.LVA(); // LVA on ARM code
         program.toARM(fname, true); // rewrite ARM code with register allocation
      }
   }
